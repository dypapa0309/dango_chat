...
export async function handler(event) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('drivers')
      .select('id, name, phone, status, dispatch_enabled, completed_jobs, rating, acceptance_rate, response_score, bank_name, account_number, account_holder, payout_enabled, payout_note, join_token, vehicle_type, vehicle_number, commercial_plate_confirmed, consign_contract_agreed, consign_contract_version, consign_contract_accepted_at, internal_memo, tax_name, tax_birth_date, tax_id_number, tax_email, tax_address, tax_withholding_type, tax_withholding_agreed, supports_move, supports_clean, supports_yd, created_at')
